    ...
    public static ArrayList<EarthQuakeData> extractEarthquakes() {
        ...
        try {
            ...
            for(int i = 0 ;i<jsonArray.length();i++)
            {
                ...
                double mag = propertyObject.getDouble("mag");
                String location = propertyObject.getString("place");
                long time = propertyObject.getLong("time");
                String url_path = propertyObject.getString("url");


                earthquakes.add(new EarthQuakeData(mag,location,time,url_path));
